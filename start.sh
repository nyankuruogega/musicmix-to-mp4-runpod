     ...
     1. TITLES: Give me exactly 5 compelling YouTube video title options for this mix. Make them engaging, include relevant keywords, and vary the
     style (some with emojis, some without, some with year/era references). Number them 1-5.
     ...
     2. DESCRIPTION: Write one complete YouTube video description that includes:
        ...
        - A line encouraging viewers to like, subscribe and turn on notifications{"for " + channel_name.strip() if channel_name and
     channel_name.strip() else ""}
     ...
     h1.app-title { font-size: 2.6rem; font-weight: 800; background: linear-gradient(135deg, #a78bfa, #60a5fa, #34d399); -webkit-background-clip:
     text; -webkit-text-fill-color: transparent; text-align: center; margin: 1.5rem 0 0.3rem; letter-spacing: -1px; }
     ...
     label { color: #a78bfa !important; font-size: 0.78rem !important; font-weight: 600 !important; text-transform: uppercase !important;
     letter-spacing: 0.08em !important; font-family: monospace !important; }
     .generate-btn { background: linear-gradient(135deg, #7c3aed, #2563eb) !important; border: none !important; border-radius: 12px !important;
     font-size: 1.05rem !important; font-weight: 700 !important; color: white !important; padding: 0.85rem 2rem !important; transition: all 0.2s
     !important; }
     .generate-btn:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 30px rgba(124,58,237,0.4) !important; }
     .ai-btn { background: linear-gradient(135deg, #059669, #0284c7) !important; border: none !important; border-radius: 12px !important; font-size:
     1.05rem !important; font-weight: 700 !important; color: white !important; padding: 0.85rem 2rem !important; transition: all 0.2s !important; }
     .ai-btn:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 30px rgba(5,150,105,0.4) !important; }
     .flux-btn { background: linear-gradient(135deg, #b45309, #92400e) !important; border: none !important; border-radius: 12px !important;
     font-size: 1.05rem !important; font-weight: 700 !important; color: white !important; padding: 0.85rem 2rem !important; transition: all 0.2s
     !important; }
     .flux-btn:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 30px rgba(180,83,9,0.4) !important; }
     .use-btn { background: linear-gradient(135deg, #7c3aed, #059669) !important; border: none !important; border-radius: 10px !important; font-size:
      0.95rem !important; font-weight: 700 !important; color: white !important; padding: 0.6rem 1.5rem !important; }
     ...
     with gr.Blocks(title="Music to MP4") as demo:
         ...
         with gr.Tabs():
             ...
             with gr.Tab("🎨 AI Background"):
                 gr.HTML("<p style='color:#6b7280;font-size:0.85rem;margin-bottom:1rem'>Generate a background image with Flux. Then click <b>Use as
     Background</b> to send it to the video generator.</p>")
                 with gr.Row():
                     with gr.Column(scale=1, elem_classes="panel"):
                         ...
                         gr.HTML("<p style='color:#6b7280;font-size:0.75rem;margin-top:0.5rem'>⚠️  Flux Dev requires HF_TOKEN env var and 24GB+ VRAM.
     Flux Schnell needs ~12GB VRAM.</p>")
             ...
             with gr.Tab("🎬 Generate Video"):
                 with gr.Row():
                     ...
                     with gr.Column(scale=1, elem_classes="panel"):
                         ...
                         visualizer = gr.Dropdown(label="Visualizer", choices=["None", "Waveform", "Spectrum", "Frequency Bars", "Circular"],
     value="Waveform")
                         resolution = gr.Dropdown(label="Resolution", choices=["1920x1080 (Full HD)", "1280x720 (HD)", "854x480 (SD)"],
     value="1280x720 (HD)")
                 ...
                 def process_with_ai_bg(zip_file, uploaded_bg, ai_bg_path, visualizer_type, video_quality, resolution,
     progress=gr.Progress(track_tqdm=True)):
                 ...
                 gr.HTML("<p style='text-align:center;color:#374151;font-size:0.78rem;margin-top:0.8rem;font-family:monospace'>Supported: MP3 · WAV ·
      OGG · FLAC · AAC · M4A · OPUS</p>")

             # ── TAB 3: AI YouTube Metadata ───────────────────────────────────────
             with gr.Tab("✨ AI YouTube Metadata"):
                 gr.HTML("<p style='color:#6b7280;font-size:0.85rem;margin-bottom:1rem'>Generate titles and a full description using your tracklist.
     Run the video generator first.</p>")
                 with gr.Row():
                     with gr.Column(elem_classes="panel"):
                         gr.Markdown("### 🔑 Setup")
                         ...
                         mix_type = gr.Textbox(label="Mix Type / Description", placeholder="e.g. Kenyan Benga Classics, 90s East African Rhumba...",
     lines=2)
